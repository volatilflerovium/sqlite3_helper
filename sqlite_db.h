 ...
 * 
 * The simplest way to execute SQL queries can be done using one of 
 ...
 * QParamV2<UTF> qParam (or QParamV3<UTF> qParam). The easy way to describe 
 * QParamV2 (QParamV3) is as follows:
 * 
 ...
 * );
 * 
 * The name parameters in sqlite3_prepare_v2 // or sqlite3_prepare16_v2
 * and in qlite3_prepare_v3 // or sqlite3_prepare16_v3 can be wrapped using
 * QParams via any of the following constructors QParams(nBytes), 
 * QParams(nBytes, pzTail), QParam(nBytes, prepFlags),
 * QParams(nBytes, prepFlags, pzTail). Example:
 * 
 * bool SQLiteDB::executeQuery(UTF query, QParams(123, tail)); // or
 * 
 * SqlRows SQLiteDB::getResultRows(UTF query, QParam(123, flags, tail));
 * 
 * void SQLiteDB::applyToRows(UTF query, SqlRowFunc callback, QParam(123));
 * 
 ...
 * 
 * A more interesting situation arises for execute secure SQL statements (bind values to
